...
def main(_):
    ...
    while True:
        ...
        if current_samples % min_required_samples != 0:
            ...
            concat_pooled_prompt_embeds = torch.cat(
                [concat_pooled_prompt_embeds, concat_pooled_prompt_embeds[random_indices]], dim=0
            )

        advantages = concat_advantages.reshape(accelerator.num_processes, -1, *concat_advantages.shape[1:])[
            accelerator.process_index
        ].to(accelerator.device)
        latents = concat_latent.reshape(accelerator.num_processes, -1, *concat_latent.shape[1:])[
            accelerator.process_index
        ].to(accelerator.device)
        prompt_embeds = concat_prompt_embeds.reshape(accelerator.num_processes, -1, *concat_prompt_embeds.shape[1:])[
            accelerator.process_index
        ].to(accelerator.device)
        pooled_prompt_embeds = concat_pooled_prompt_embeds.reshape(
            accelerator.num_processes, -1, *concat_pooled_prompt_embeds.shape[1:]
            ...
            for i, sample in tqdm(
                ...
                for j in tqdm(
                    ...
                    with accelerator.accumulate(transformer):
                        ...
                        theta_mse = (
                            ((model_pred.float() - target.float()) ** 2).reshape(target.shape[0], -1).mean(dim=1)
                        )
                        ref_mse = (
                            ((model_pred_ref.float() - target.float()) ** 2).reshape(target.shape[0], -1).mean(dim=1)
                        )
